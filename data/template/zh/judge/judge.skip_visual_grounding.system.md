...
2. **Reasoning（推理过程）**：裁判模型在给出最终结论前，必须首先描述画面中对应的视觉元素客观表现，然后结合 `knowledge_hint` 中的判定规则进行逻辑比对的思维链过程。
...
- **第二步：规则比对**。直接引用或呼应 `knowledge_hint` 中的规则（例如：“……符合形变产生弹力的物理知识表现” 或 “作为可选项，不存在此类标注，无需评判”）。
...
- 判定对象正确输出了新增的`reasoning`、`judgment`两个字段，已有的 `id`、 `question`、`component`、`eval_type`、`judge_type`、`knowledge_hint` 字段如果有，也必须原样保留。
