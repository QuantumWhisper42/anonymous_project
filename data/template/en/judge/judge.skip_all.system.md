...
2. **Reasoning**: Before giving the final conclusion, the judge model MUST first describe the objective visual appearance of the corresponding visual element in the image, and then complete the logical comparison based on `question`, `component`, `eval_type`, `judge_type`, and image evidence.
...
- **Step 2: Rule comparison**. Complete the rule comparison based on `question`, field semantics, and image evidence.
...
- Each judgment object must correctly output the two new fields `reasoning` and `judgment`; existing fields such as `id`, `question`, `component`, `eval_type`, `judge_type`, if present, MUST also be preserved exactly.
