...
2. **Reasoning**: Before giving the final conclusion, the judge model MUST first describe the objective visual appearance of the corresponding visual element in the image, and then compare it logically against the judgment rules in `knowledge_hint`. If `visual_grounding` is provided, also compare against the visual grounding reference content and explain which key features are missing, inconsistent, or logically non-compliant.
...
- For `VISUAL` and `INSTRUCTION` type judgments, compare only object color, shape, quantity, relative position, and environmental features.
...
- **Step 2: Rule comparison**. Directly quote or echo the rule in `knowledge_hint` (for example: "... conforms to the physical knowledge representation that deformation produces elastic force" or "As an optional item, this type of annotation does not exist, so no judgment is needed"). If `visual_grounding` exists, also compare against the visual grounding reference content and explain which key features are missing, inconsistent, or logically non-compliant.
...
2. **Item-by-item review**: Traverse the input VQA Checklist and, for each `question`, analyze:
   ...
   - Is it whether it is a general visual verification of the world-knowledge carrier (`VISUAL`), general instruction following (`INSTRUCTION`), or knowledge verification (`KNOWLEDGE`)?
   - Where are the PASS and FAIL red lines given by `knowledge_hint`?
   - Which key features provided by the `visual_grounding` reference content are missing, inconsistent, or logically non-compliant?
3. **Visual grounding and alignment**: With the questions above, return to the image to look for evidence. If no evidence is found, this means failure for `REQUIRED` and not applicable for `OPTIONAL`.
  - If `visual_grounding` exists, also compare against the visual grounding reference content and explain which key features are missing, inconsistent, or logically non-compliant.
...
- Each judgment object must correctly output the two new fields `reasoning` and `judgment`; existing fields such as `id`, `question`, `component`, `eval_type`, `judge_type`, `knowledge_hint`, `visual_grounding`, if present, MUST also be preserved exactly.
...
[
  {
    ...
    "judge_type": "REQUIRED",
    "knowledge_hint": "Pure instruction following. The Prompt explicitly asks for "one" component made of a glass container. If multiple containers or piles of glass bottles appear in the image, the quantity-constraint instruction is not followed and it should FAIL.",
...
```json
{
  "vqa_checklist": [
    {
      "question": "The image contains exactly one main glass-container entity",
      "reasoning": "Observing the image, a glass-container entity is clearly present, and its quantity is exactly one. This satisfies the pass condition.",
      "judgment": "PASS"
    },
    {
      "id": 0,
