...
2. **Reasoning**: Before giving the final conclusion, the judge model MUST first describe the objective visual appearance of the corresponding visual element in the image, and then complete the logical comparison based on `question`, `component`, `eval_type`, `judge_type`, and image evidence. If `visual_grounding` is provided, also compare against the visual grounding reference content and explain which key features are missing, inconsistent, or logically non-compliant.
...
- **For `REQUIRED` items**:
  ...
  - **Judgment rule**: If the image evidence is consistent with `question`, the semantics of the existing fields, and the task statement, judge it as `PASS`; if the subject is missing from the image, or the subject's appearance is clearly inconsistent with the task meaning, you MUST judge it as `FAIL`.
...
- **Step 2: Rule comparison**. Complete the rule comparison based on `question`, field semantics, and image evidence. If `visual_grounding` exists, also compare against the visual grounding reference content and explain which key features are missing, inconsistent, or logically non-compliant.
...
2. **Item-by-item review**: Traverse the input VQA Checklist and, for each `question`, analyze:
   ...
   - Carefully determine the PASS and FAIL boundaries based on `question`, `component`, `eval_type`, `judge_type`, and image evidence.
...
- Each judgment object must correctly output the two new fields `reasoning` and `judgment`; existing fields such as `id`, `question`, `component`, `eval_type`, `judge_type`, `visual_grounding`, if present, MUST also be preserved exactly.
...
- [ ] **Reasoning coherence check**: Does `reasoning` clearly describe "what is seen in the image"?
